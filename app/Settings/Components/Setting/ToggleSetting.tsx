...
const ToggleSetting = ({title, defaultValue}: Props) => {
  ...
  return (
    <li className='flex'>
        <button className="flex-1 flex" onClick={() => handlePreferenceChange()}>{title}</button>
        <button onClick={() => handlePreferenceChange()} className={`flex w-12 h-6 duration-300 transition-all rounded-full ${active ? 'bg-[#B23928]' : 'bg-gray-600'}`}>
